...
export const Player: React.FC = () => {
  ...
  const { status, laneCount, takeDamage, activateImmortality, isImmortalityActive, immortalityEndsAt } = useStore();
  ...
  const triggerJump = () => {
    const maxJumps = 2; // Shop removed: double jump always available
  ...
  }, [status, laneCount, activateImmortality]);

  // MQTT / external controller hook (more reliable than synthetic KeyboardEvent in some browsers)
  useEffect(() => {
    const handleGameControl = (e: Event) => {
      if (status !== GameStatus.PLAYING) return;
      const detail = (e as CustomEvent).detail as { cmd?: 'up' | 'left' | 'right' } | undefined;
      const cmd = detail?.cmd;
      const maxLane = Math.floor(laneCount / 2);

      if (cmd === 'left') setLane(l => Math.max(l - 1, -maxLane));
      else if (cmd === 'right') setLane(l => Math.min(l + 1, maxLane));
      else if (cmd === 'up') triggerJump();
    };

    window.addEventListener('game-control', handleGameControl as EventListener);
    return () => window.removeEventListener('game-control', handleGameControl as EventListener);
  }, [status, laneCount]);
  ...
  useFrame((state, delta) => {
    ...
    // Shield ring visual: fade out as immortality time elapses
    if (shieldRef.current) {
        const mat = shieldMaterial as THREE.MeshBasicMaterial;
        if (isImmortalityActive && immortalityEndsAt > 0) {
            const remainingMs = Math.max(0, immortalityEndsAt - Date.now());
            const tLeft = Math.min(1, remainingMs / 5000);
            // Fade: strong at start, near invisible at end
            mat.opacity = 0.55 * tLeft;
            // Gentle breathing + slight shrink over time
            const pulse = 1 + Math.sin(state.clock.elapsedTime * 8) * 0.04;
            const shrink = 0.85 + 0.15 * tLeft;
            shieldRef.current.scale.setScalar(pulse * shrink);
  ...
  return (
    <group ref={groupRef} position={[0, 0, 0]}>
      ...
      <group ref={shieldRef} position={[0, 1.0, 0]}>
        ...
        <mesh geometry={SHIELD_RING_GEO} rotation={[0, 0, 0]} material={shieldMaterial} />
      </group>
